...
prompts:
        ...
        correct_opcode:
                ...
                jr		$ra		
error:
        missing_colon: la	$a0, missing_colon_prompt               # Load and print string asking for string
                li 	$v0, 4
                syscall
                j	ending				        # jump to ending
        invalid_opcode: la      $a0, invalid_opcode_prompt
                li      $v0, 4
                syscall
                j	ending				        # jump to ending
        too_many_variable: move	$a0, $s0               # Load and print string asking for string
                 li 	$v0, 1
                 syscall
                ...
                syscall
                j       ending
        not_valid_register: la      $a0, not_valid_register_prompt 
                li      $v0, 4
                syscall
                j       ending
        not_num_error:
        ...
        missing_:
                ...
                syscall
                j       ending
valid_syntax: la      $a0, valid_syntax_prompt
        li      $v0, 4
        syscall
ending: la      $a0, continue_prompt
        li      $v0, 4
        syscall 
        li      $v0, 5
        syscall 
...
clean_block: li $a0, 0 
        li $a1, 0
        la      $s0, buffer
loop_block: beq		$a1, 100, jump_
        sb              $a0, 0($s0)
        addi            $s0, $s0, 1
        addi            $a1, $a1, 1
        j               loop_block
clean_opcode: li $a0, 0 
        li $a1, 0
        la      $s1, opcode
loop_opcode:
        beq		$a1, 10, jump_
